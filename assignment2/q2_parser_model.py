...
class ParserModel(Model):
    ...
    def add_prediction_op(self):
        ...
        W = tf.Variable(xavier_initializer((self.config.hidden_size, self.config.n_features * self.config.embed_size)))
        U = tf.Variable(xavier_initializer((self.config.n_classes, self.config.hidden_size)))
        b1 = tf.Variable(tf.zeros([self.config.hidden_size]))
        b2 = tf.Variable(tf.zeros([self.config.n_classes]))
        
        h = tf.add(tf.matmul(x, W, transpose_b=True), b1)
        ha = tf.nn.relu(h)
        h_drop = tf.nn.dropout(ha, 1 - self.dropout_placeholder)
        pred = tf.add(tf.matmul(h_drop, U, transpose_b=True), b2)
        ### END YOUR CODE
        return pred
    ...
    def add_loss_op(self, pred):
        ...
        loss = tf.reduce_mean(loss) # average loss of minibatch
        ### END YOUR CODE
        return loss
    ...
    def run_epoch(self, sess, parser, train_examples, dev_set):
        ...
        # Mark below code for the keras issue
        # https://github.com/openai/gym/issues/905
