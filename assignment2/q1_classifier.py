...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        """
        ### YOUR CODE HERE
        #self.W = tf.get_variable("W", [self.config.n_classes, self.config.n_features], initializer=tf.zeros_initializer)
        #self.b = tf.get_variable("b", self.config.n_classes, initializer=tf.zeros_initializer)
        self.W = tf.Variable(tf.zeros([self.config.n_classes, self.config.n_features]))
        self.b = tf.Variable(tf.zeros([self.config.n_classes]))
        pred = softmax(tf.add(tf.matmul(self.input_placeholder, self.W, transpose_b=True), self.b))
        ### END YOUR CODE
        return pred
